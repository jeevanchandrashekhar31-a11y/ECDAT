...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
 ...
 return (
 <div className="flex h-screen print:h-auto bg-background text-text-brand font-sans selection:bg-brand/20 selection:text-brand">
 {/* Sidebar Navigation */}
 <aside className="w-64 glass-panel m-4 flex flex-col z-20 overflow-hidden shrink-0 print:hidden">
 {/* Branding */}
 <div className="p-6 border-b border-border/50">
 <div className="flex items-center gap-3">
 <div className="p-2.5 rounded-xl from-primary to-secondary text-text-brand flex-shrink-0">
 <Shield size={24} className="animate-float" />
 </div>
 <div>
 <div className="flex items-center gap-2">
 <h1 className="text-xl font-display font-bold tracking-tight text-text-brand m-0 leading-none">
 ECDAT
 </h1>
 <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-brand/20 border border-brand/30 text-brand font-bold uppercase tracking-wider">
 PQC
 </span>
 </div>
 <p className="text-[10px] text-text-secondary mt-1 font-medium tracking-wide">Enterprise Crypto Discovery</p>
 </div>
 </div>
 </div>
 ...
 {!currentUser ? (
 <div className="mb-3 p-2 rounded-xl bg-surface-2 /15 border border-high text-center">
 <NavLink
 to="/login"
 className="w-full inline-flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-surface-2 to-cyan-500 text-text-muted font-bold text-xs shadow-md hover:brightness-110 transition-all"
 >
 <ShieldCheck size={15} />
 ...
 <div className="mb-3 px-3 py-1.5 rounded-lg bg-success/15 border border-success flex items-center justify-between text-2xs text-success font-semibold">
 ...
 : 'text-text-secondary hover:text-text-brand hover:bg-surface-2/50'
 }`
 }
 >
 ...
 isActive
 ? 'bg-pqc text-text-muted shadow-md font-bold'
 : 'text-text-secondary hover:text-text-brand hover:bg-surface'
 }`
 }
 >
 <Network size={17} />
 ...
 className="w-full flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-surface hover:bg-surface-2 text-crypto border border-border-soft font-bold transition-all shadow-sm"
 >
 <Upload size={16} />
 ...
 <div className="glass-card p-3 bg-background/60">
 <div className="flex items-center gap-2 text-text-secondary mb-1">
 <ShieldCheck size={14} className="text-crypto" />
 <span className="font-semibold text-text-brand">CycloneDX 1.6</span>
 </div>
 ...
 {currentUser?.isEvaluation && (
 <div className=" bg-surface-2 border-b border-indigo-500/40 px-6 py-2 flex items-center justify-center text-xs text-indigo-200">
 ...
 <div className="flex items-center gap-3">
 <span className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Active Scan:</span>
 <div className="relative">
 <Zap className="w-3.5 h-3.5 text-high absolute left-3 top-1/2 -translate-y-1/2 z-10" />
 <select
 className="pl-9 pr-8 py-2 bg-surface border border-border-soft/50 text-text-brand text-xs font-semibold rounded-lg appearance-none cursor-pointer hover:bg-surface transition-colors focus:outline-none focus:ring-2 focus:ring-1 ring-border"
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold border transition-all bg-success/15 text-success border-success hover:bg-success/25 cursor-pointer"
 >
 ...
 {/* Dropdown */}
 {personaDropdownOpen && (
 <div className="absolute right-0 top-full mt-1 w-56 rounded-xl bg-bg-1 border border-border-soft shadow-md z-50 py-1">
 <div className="px-3 py-2 border-b border-border text-2xs uppercase tracking-wider text-text-muted font-bold">
 ...
 className="w-full text-left px-4 py-2 text-xs text-text-secondary hover:bg-surface hover:text-crypto"
 >
 Security Analyst
 </button>
 <button
 ...
 <div className="px-4 py-2 text-xs text-indigo-400 hover:bg-surface font-semibold cursor-pointer">
 Evaluation Guide
 </div>
 <button
 onClick={async () => {
 ...
 className="w-full text-left px-4 py-2 text-xs text-high hover:bg-surface font-semibold"
 >
 Reset Evaluation
 </button>
 <button
 ...
 className="w-full text-left px-4 py-2 text-xs text-critical hover:bg-surface font-semibold"
 >
 Exit Evaluation
 </button>
 </div>
 ...
 isActive
 ? 'bg-pqc/20 text-crypto border-pqc/40'
 : currentUser
 ? 'bg-pqc/15 text-crypto border-pqc/30 hover:bg-pqc/25'
 ...
 <div className="bg-surface-2 border-b border-critical px-6 py-2.5 flex items-center justify-between text-xs text-critical animate-in slide-in-from-top duration-200">
 <div className="flex items-center gap-2.5">
 <div className="p-1 rounded-md bg-surface-2 text-critical border border-critical">
 <AlertTriangle size={15} />
 </div>
 <span className="font-bold text-text-brand tracking-wide">
 Security Defense Alert ({securityAlert.status}):
 </span>
 <span className="text-critical">{securityAlert.message}</span>
 </div>
 <div className="flex items-center gap-2">
 <button
 onClick={() => navigate('/login')}
 className="px-3 py-1 rounded bg-surface-2 hover:bg-surface-2 text-critical font-semibold text-2xs transition-colors border border-critical"
 >
 Authenticate Now
 </button>
 <button
 onClick={() => setSecurityAlert(null)}
 className="p-1 text-critical hover:text-text-brand transition-colors"
 title="Dismiss alert"
 >
 <X size={14} />
 </button>
