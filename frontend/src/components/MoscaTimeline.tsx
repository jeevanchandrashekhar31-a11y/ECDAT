    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-white tracking-wide">Mosca Theorem Quantum Risk Engine</h2>
                {/* Visual Distinction: Stored vs Simulation Badge */}
                {isSimulated ? (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/15 text-amber-300 border border-amber-500/40 shadow-sm animate-pulse">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
          ...
          <div className="text-2xl font-bold font-mono text-purple-300">
            {localY.toFixed(1)} <span className="text-xs font-normal text-slate-400">yrs</span>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
          ...
          <div className="text-2xl font-bold font-mono text-indigo-300">
            {totalXY.toFixed(1)} <span className="text-xs font-normal text-slate-400">yrs</span>
          </div>
          <p className="text-[11px] text-slate-500">Total transition lifespan</p>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
          ...
          <div className="text-2xl font-bold font-mono text-amber-300">
            {currentZ.toFixed(1)} <span className="text-xs font-normal text-slate-400">yrs</span>
          </div>
          <p className="text-[11px] text-slate-500">~{SCENARIO_CONFIG[localScenario]?.horizonYear} horizon</p>
        </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-950/80 border border-slate-800/90 space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="relative pt-8 pb-4">
            ...
            <div className="h-10 bg-slate-900/90 rounded-xl border border-slate-800 relative overflow-hidden flex items-center shadow-inner">
              ...
              <div
                className="h-full bg-gradient-to-r from-cyan-600 to-cyan-500 opacity-90 transition-all duration-300 relative group flex items-center justify-center"
                style={{
                  width: `${xPercent}%`,
                }}
              >
                {xPercent > 10 && (
                  <span className="text-[11px] font-bold text-slate-950 font-mono truncate px-1">
              ...
              <div
                className="h-full bg-gradient-to-r from-purple-600 to-indigo-500 opacity-90 transition-all duration-300 relative group flex items-center justify-center"
                style={{
                  width: `${Math.max(0, totalPercent - xPercent)}%`,
                }}
              >
                {totalPercent - xPercent > 10 && (
                  <span className="text-[11px] font-bold text-white font-mono truncate px-1">
              ...
              {totalXY > currentZ && (
                <div
                  className="absolute top-0 bottom-0 bg-rose-500/25 border-y-2 border-rose-500/60 animate-pulse flex items-center justify-center"
                  style={{
                    left: `${zPercent}%`,
                    width: `${totalPercent - zPercent}%`,
                  }}
                >
                  <span className="text-[10px] font-mono font-black text-rose-300 px-1 truncate drop-shadow">
              ...
              {totalXY <= currentZ && (
                <div
                  className="absolute top-0 bottom-0 bg-emerald-500/15 border-y border-emerald-500/40 flex items-center justify-center"
                  style={{
                    left: `${totalPercent}%`,
                    width: `${zPercent - totalPercent}%`,
                  }}
                >
                  <span className="text-[10px] font-mono font-semibold text-emerald-400 px-1 truncate">
            ...
            >
              {/* Top Z Flag */}
              <div className="absolute -top-7 -translate-x-1/2 flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-500 text-slate-950 font-bold font-mono text-[10px] shadow-lg whitespace-nowrap">
                <Zap className="w-3 h-3 text-slate-950 fill-current" />
                <span>Z = {currentZ.toFixed(1)}y (CRQC)</span>
              </div>
              {/* Vertical line through bar */}
              <div className="w-0.5 h-full bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.8)]"></div>
            </div>
            ...
            >
              {/* Bottom (X+Y) Flag */}
              <div className="absolute -bottom-7 -translate-x-1/2 flex items-center gap-1 px-2 py-0.5 rounded-md bg-indigo-600 text-white font-bold font-mono text-[10px] shadow-lg whitespace-nowrap">
                <span>X+Y = {totalXY.toFixed(1)}y</span>
              </div>
              {/* Vertical line */}
              <div className="w-0.5 h-full bg-indigo-400 shadow-[0_0_8px_rgba(129,140,248,0.8)]"></div>
            </div>
          </div>

          {/* Scale Axis & Tick Labels */}
          <div className="relative h-6 text-slate-400 font-mono text-[10px]">
            {ticks.map((tick) => {
              const leftPercent = (tick / maxScale) * 100;
              return (
                <div
                  ...
                  style={{ left: `${leftPercent}%` }}
                >
                  <div className="w-0.5 h-1.5 bg-slate-700"></div>
          ...
          {/* Legend */}
          <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 pt-2 text-xs border-t border-slate-900">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-cyan-500"></div>
              <span className="text-slate-300">
                X: Confidentiality Lifetime (<strong className="text-white font-mono">{currentX.toFixed(1)}y</strong>)
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-purple-500"></div>
              <span className="text-slate-300">
                Y: Migration Duration (<strong className="text-white font-mono">{localY.toFixed(1)}y</strong>)
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-amber-400"></div>
              <span className="text-slate-300">
                Z: Threat Horizon (<strong className="text-white font-mono">{currentZ.toFixed(1)}y</strong>)
              </span>
            </div>
            {totalXY > currentZ ? (
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded bg-rose-500/80 border border-rose-400 animate-pulse"></div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded bg-emerald-500/80 border border-emerald-400"></div>
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <p className="leading-relaxed text-slate-200">{statusMeta.description}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          ...
          <span className="text-[11px] text-slate-400">Client-side interactive simulation (safe, non-destructive)</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-3 p-4 rounded-xl bg-slate-900/60 border border-slate-800/80">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold font-mono text-purple-400 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-800/50">
          ...
          <div className="space-y-3 p-4 rounded-xl bg-slate-900/60 border border-slate-800/80">
            ...
            <p className="text-[11px] text-slate-500">Switches quantum cryptanalysis emergence timeline benchmark.</p>
          </div>
        </div>

        {/* Simulation Diff Review Bar */}
        {isSimulated && (
          <div className="p-3.5 rounded-xl bg-amber-950/20 border border-amber-900/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-amber-400 shrink-0" />
              <div className="text-slate-300">
                <span className="font-bold text-amber-300">Active What-If Simulation:</span> Stored Assessment:{' '}
            ...
            <button
              onClick={handleReset}
              className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg transition-colors text-xs shrink-0 cursor-pointer"
            >
        ...
        {policyProfile && (
          <div className="text-[11px] text-slate-500 font-mono text-right pt-2 border-t border-slate-900">
