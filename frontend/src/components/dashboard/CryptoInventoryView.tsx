...
export const CryptoInventoryView: React.FC<Props> = ({
 ...
 <h3 className="text-lg font-bold text-text-brand">Cryptographic Inventory (CBOM)</h3>
 </div>
 ...
 className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-surface-2 border border-border-soft text-crypto hover:bg-surface-2 transition-colors inline-flex items-center gap-1.5"
 >
 ...
 : 'bg-bg-1 border border-border text-text-secondary hover:text-text-brand'
 }`}
 >
 All Primitives
 </button>
 ...
 <div className="font-semibold text-text-brand flex items-center gap-1.5">
 <KeyRound className="w-3.5 h-3.5 text-crypto shrink-0" />
 <span>{comp.name}</span>
 </div>
 <span className="font-mono text-[10px] text-text-muted">{comp.id}</span>
 </td>
 <td className="py-3 px-4 font-mono text-text-brand">
 {comp.key_size ? `${comp.key_size} bit` : 'N/A'}
 </td>
 <td className="py-3 px-4">
 <span className="px-2 py-0.5 rounded bg-surface text-text-secondary text-[11px] font-medium border border-border-soft">
 {comp.primitive}
 </span>
 </td>
 <td className="py-3 px-4">
 ...
 {comp.line_number && (
 <span className="text-crypto">:{comp.line_number}</span>
 )}
 </div>
 ) : (
 'Network / Dynamic'
