...
export const Remediation: React.FC = () => {
 ...
 case 'REVIEWED':
 return 'bg-brand/20 text-brand border-brand';
 case 'APPROVED':
 return 'bg-specialized text-specialized border-specialized';
 case 'APPLIED':
 return 'bg-pqc/20 text-crypto border-pqc/40';
 case 'VERIFIED':
 return 'bg-success/20 text-success border-success font-bold';
 case 'REJECTED':
 case 'ROLLED_BACK':
 ...
 <Wrench className="text-crypto" size={22} />
 ...
 <div className="flex items-center gap-1.5 p-1 rounded-xl bg-bg-1 border border-border text-xs">
 <span className="text-text-secondary text-2xs px-2 font-medium">Active Persona:</span>
 <span className="text-crypto font-bold px-2 py-1 text-2xs uppercase tracking-wider">{authManager.getSession().role || 'Analyst'}</span>
 </div>

 <button
 onClick={() => setProposeModalOpen(true)}
 className="px-3.5 py-2 rounded-xl bg-brand hover:bg-brand text-text-muted font-bold text-xs shadow-md flex items-center gap-1.5 transition-all"
 >
 <Plus size={15} />
 ...
 {/* Alerts */}
 {actionError && (
 <div className="p-4 rounded-xl bg-critical/10 border border-critical text-critical text-xs flex items-start justify-between gap-3">
 ...
 {actionError.code && (
 <p className="font-mono text-2xs text-critical mt-1">Code: {actionError.code}</p>
 )}
 {actionError.message.includes('Four-Eyes') && (
 <div className="mt-2 pt-2 border-t border-critical flex items-center gap-2">
 <span className="text-text-secondary">Use the Evaluation Persona dropdown in the header to switch to a Security Approver to satisfy role-separation.</span>
 </div>
 )}
 </div>
 </div>
 <button onClick={() => setActionError(null)} className="text-critical hover:text-text-brand">
 <XCircle size={14} />
 </button>
 </div>
 )}
 ...
 {actionSuccess && (
 <div className="p-3.5 rounded-xl bg-success/10 border border-success text-success text-xs flex items-center justify-between gap-2">
 <div className="flex items-center gap-2">
 <CheckCircle size={16} className="text-success shrink-0" />
 <span>{actionSuccess}</span>
 </div>
 <button onClick={() => setActionSuccess(null)} className="text-success hover:text-text-brand">
 <XCircle size={14} />
 </button>
 </div>
 )}
 ...
 <h3 className="text-sm font-bold text-text-brand mb-1">Loading Remediation Proposals</h3>
 ...
 <p className="text-xs text-critical">{listError.message}</p>
 <button
 ...
 <h3 className="text-base font-bold text-text-brand mb-1">No Remediation Proposals Recorded</h3>
 ...
 <button onClick={() => fetchApprovals()} className="hover:text-crypto">
 <RefreshCw size={12} />
 </button>
 </div>
 ...
 isSelected
 ? 'bg-surface border-pqc/50 '
 : 'bg-bg-1/60 border-border hover:bg-surface-2 hover:border-border-soft'
 }`}
 >
 ...
 <span className="text-text-muted font-mono text-2xs truncate">
 {appr.approval_id}
 </span>
 </div>

 <h3 className="text-xs font-bold text-text-brand mb-1 leading-snug">{appr.title}</h3>
 <p className="text-text-secondary text-2xs line-clamp-2 mb-3">{appr.description || 'No description provided.'}</p>

 <div className="flex items-center justify-between text-2xs text-text-secondary pt-2 border-t border-border">
 <span>Proposer: <strong className="text-text-secondary">{appr.proposer?.username}</strong></span>
 ...
 <div className="glass-card p-6 rounded-2xl border border-border bg-bg-1/60 shadow-xl space-y-6">
 {/* Proposal Header */}
 ...
 <span className="text-text-secondary text-xs font-mono">ID: {selectedApproval.approval_id}</span>
 </div>
 <h2 className="text-base font-bold text-text-brand">{selectedApproval.title}</h2>
 <p className="text-xs text-text-secondary mt-1">{selectedApproval.description}</p>
 </div>

 <span className="px-2.5 py-1 rounded-lg bg-background border border-border text-2xs font-mono text-crypto">
 ...
 <div className="p-4 rounded-xl bg-surface-2 border border-specialized space-y-2 mt-4">
 <div className="flex items-center gap-2 text-specialized font-bold text-xs">
 <Sparkles size={14} />
 <span>Original AI Security Assessment</span>
 <span className="ml-auto px-1.5 py-0.5 rounded bg-surface-2 text-specialized border border-specialized text-[9px] uppercase font-bold">
 Tier 2 Finding
 </span>
 </div>
 <p className="text-xs text-text-secondary italic border-l-2 border-specialized pl-3 leading-relaxed">
 ...
 isComplete
 ? 'bg-surface-2 border-pqc/60 text-crypto font-bold'
 ...
 <Lock size={14} className="text-crypto" />
 <span>Next Governance Transition Action</span>
 </span>
 <span className="text-2xs text-text-secondary">
 Active: <strong className="text-crypto font-mono">{activePersona}</strong>
 </span>
 </div>
 ...
 className="w-full py-2.5 px-4 rounded-xl bg-pqc hover:bg-pqc text-text-muted font-bold text-xs flex items-center justify-center gap-2 "
 >
 ...
 <div className="p-2.5 rounded-lg bg-high/10 border border-high text-high text-2xs flex items-center gap-2">
 ...
 disabled={actionLoading}
 className="flex-1 py-2.5 px-4 rounded-xl bg-specialized hover:bg-specialized text-text-brand font-bold text-xs flex items-center justify-center gap-2 shadow-violet-600/20"
 >
 ...
 disabled={actionLoading}
 className="w-full py-2.5 px-4 rounded-xl bg-brand hover:bg-brand text-text-muted font-bold text-xs flex items-center justify-center gap-2 "
 >
 ...
 disabled={actionLoading}
 className="w-full py-2.5 px-4 rounded-xl bg-success hover:bg-success text-text-muted font-bold text-xs flex items-center justify-center gap-2 "
 >
 ...
 <div className="p-4 rounded-xl bg-surface-2 border border-success space-y-3">
 ...
 <div className="bg-background p-3 rounded-lg border border-border text-2xs font-mono text-success space-y-1">
 ...
 <div>
 <h4 className="text-2xs font-semibold uppercase tracking-wider text-text-muted mb-2 flex items-center gap-1.5">
 <Code2 size={13} className="text-crypto" />
 <span>Proposed Cryptographic Diff</span>
 </h4>
 <div className="bg-background rounded-xl border border-border p-3 font-mono text-2xs text-crypto overflow-x-auto max-h-48">
 ...
 <div>
 <h4 className="text-2xs font-semibold uppercase tracking-wider text-text-muted mb-2 flex items-center gap-1.5">
 <History size={13} className="text-crypto" />
 ...
 <div>
 <span className="text-crypto font-bold">{evt.to_state}</span>
 <span className="text-text-muted ml-2">by {evt.actor} ({evt.role})</span>
 <p className="text-text-secondary text-3xs font-sans mt-0.5">{evt.comments}</p>
 </div>
 <span className="text-text-muted truncate max-w-[140px]" title={evt.hash}>
 ...
 <div className="w-full max-w-lg bg-bg-1 border border-border rounded-2xl p-6 shadow-md space-y-4 max-h-[90vh] overflow-y-auto">
 <div className="flex items-center justify-between pb-3 border-b border-border">
 <h3 className="text-sm font-bold text-text-brand flex items-center gap-2">
 <Wrench size={16} className="text-crypto" />
 ...
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Proposal Title</label>
 <input
 type="text"
 value={proposeTitle}
 onChange={(e) => setProposeTitle(e.target.value)}
 required
 className="w-full bg-background border border-border rounded-xl px-3 py-2 text-text-brand focus:outline-none focus:border-pqc font-medium"
 />
 </div>
 ...
 className="w-full bg-background border border-border rounded-xl px-3 py-2 text-text-brand focus:outline-none focus:border-pqc"
 >
 ...
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Environment</label>
 <select
 value={proposeEnv}
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Target Standard</label>
 <input
 type="text"
 value={proposeTargetStandard}
 onChange={(e) => setProposeTargetStandard(e.target.value)}
 className="w-full bg-background border border-border rounded-xl px-3 py-2 text-text-brand focus:outline-none focus:border-pqc font-mono"
 />
 </div>
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Finding ID (Optional)</label>
 <input
 type="text"
 ...
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Affected Asset / File</label>
 <input
 type="text"
 value={proposeAsset}
 ...
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Patch Diff</label>
 <textarea
 value={proposePatchDiff}
 onChange={(e) => setProposePatchDiff(e.target.value)}
 rows={4}
 className="w-full bg-background border border-border rounded-xl p-2.5 font-mono text-2xs text-crypto focus:outline-none focus:border-pqc"
 />
 </div>

 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Test Plan</label>
 <input
 type="text"
 ...
 <div>
 <label className="block text-text-secondary font-semibold mb-1">Rollback Plan</label>
 <input
 type="text"
 ...
 type="button"
 onClick={() => setProposeModalOpen(false)}
 className="px-4 py-2 rounded-xl bg-surface hover:bg-surface-2 text-text-secondary font-semibold"
 >
 Cancel
 </button>
 <button
 type="submit"
 disabled={actionLoading}
 className="px-5 py-2 rounded-xl bg-brand hover:bg-brand text-text-muted font-bold shadow-md flex items-center gap-2"
 >
