...
export const Dashboard: React.FC = () => {
 ...
 <Sliders className="w-3.5 h-3.5 text-crypto" />
 <span className="text-text-secondary hidden sm:inline">Profile:</span>
 <select
 value={selectedPolicy}
 onChange={(e) => setSelectedPolicy(e.target.value)}
 className="bg-transparent text-text-brand focus:outline-none cursor-pointer font-medium"
 >
 <option value="regulated_bfsi" className="bg-bg-1 text-text-brand">
 Regulated BFSI (Strict)
 </option>
 <option value="pci_dss_v4" className="bg-bg-1 text-text-brand">
 PCI-DSS v4.0
 </option>
 <option value="nist_sp_800_56c" className="bg-bg-1 text-text-brand">
 NIST SP 800-56C
 </option>
 <option value="general_saas" className="bg-bg-1 text-text-brand">
 ...
 {/* Scenario Selector */}
 <div className="flex items-center gap-1.5 bg-background/80 px-3 py-1.5 rounded-lg border border-border text-xs">
 <Atom className="w-3.5 h-3.5 text-specialized" />
 <span className="text-text-secondary hidden sm:inline">CRQC:</span>
 <select
 value={selectedScenario}
 onChange={(e) => setSelectedScenario(e.target.value)}
 className="bg-transparent text-text-brand focus:outline-none cursor-pointer font-medium"
 >
 <option value="baseline_2030" className="bg-bg-1 text-text-brand">
 Baseline (2033)
 </option>
 <option value="aggressive_2026" className="bg-bg-1 text-text-brand">
 Aggressive (2026)
 </option>
 <option value="store_now_decrypt_later" className="bg-bg-1 text-text-brand">
 SNDL Immediate
 </option>
 </select>
 </div>
 ...
 className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-surface-2 hover:bg-surface-2 text-crypto border border-border-soft inline-flex items-center gap-1.5 transition-colors"
 >
 ...
 {/* Collapsible Quick Scan Operations Drawer */}
 {showScanDrawer && (
 <div className="glass-card p-5 border-pqc bg-background/70 animate-in fade-in slide-in-from-top-2 duration-200">
 <div className="flex items-center justify-between mb-3">
 <h4 className="text-sm font-semibold text-text-brand flex items-center gap-2">
 <Play className="w-4 h-4 text-crypto" />
 <span>Operational Scanners & CBOM Pipeline</span>
 </h4>
 <span className="text-xs text-text-muted">Run quick discovery on network endpoint or test targets</span>
 </div>
 ...
 className="w-full px-3 py-2 rounded-lg text-xs bg-bg-1 border border-border text-text-brand placeholder-slate-500 focus:outline-none focus:border-pqc font-mono"
 />
 </div>
 <button
 onClick={handleQuickNetworkScan}
 disabled={Boolean(runningAction) || !networkTarget}
 className="px-4 py-2 rounded-lg text-xs font-semibold bg-pqc hover:bg-pqc text-text-muted font-bold disabled:opacity-50 inline-flex items-center gap-1.5 transition-colors"
 >
 ...
 <Link
 to="/scans"
 className="px-3 py-2 rounded-lg text-xs bg-surface hover:bg-surface-2 text-text-secondary border border-border-soft transition-colors inline-flex items-center gap-1"
 >
 ...
 ? 'bg-surface-2 border border-success text-success'
 : 'bg-surface-2 border border-critical text-critical'
 }`}
 >
 {actionFeedback.type === 'success' ? (
 ...
 isActive
 ? 'bg-surface-2 text-crypto border border-border-soft shadow-md '
 : 'text-text-secondary hover:text-text-brand hover:bg-bg-1/60 border border-transparent'
 }`}
 >
 <Icon className={`w-4 h-4 ${isActive ? 'text-crypto' : 'text-text-muted'}`} />
 <span>{tab.label}</span>
 {tab.badge !== undefined && (
 <span
 className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${
 isActive
 ? 'bg-surface-2 text-crypto border border-border-soft'
 ...
 <h3 className="text-base font-bold text-critical">Dashboard Loading Failure</h3>
