...
export const Assets: React.FC = () => {
  ...
  const getSourceBadge = (source?: string) => {
    const s = (source || 'static').toLowerCase();
    if (s.includes('network')) {
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono bg-cyan-950/60 text-cyan-300 border border-cyan-800/40">
          network
        </span>
      );
    }
    if (s.includes('binary') || s.includes('container')) {
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono bg-purple-950/60 text-purple-300 border border-purple-800/40">
          binary
        </span>
      );
    }
  ...
  const formatDate = (dateStr?: string) => {
    if (!dateStr) return 'Active Scan';
    try {
      const d = new Date(dateStr);
      if (isNaN(d.getTime())) return 'Active Scan';
      ...
      <div className="glass-card p-4 space-y-3">
        <div className="flex items-center justify-between text-xs font-semibold text-slate-300">
          <div className="flex items-center gap-2">
            ...
            <span>Multi-Dimensional Filtering &amp; Search</span>
          </div>
          <button
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-900/90 text-slate-400 uppercase tracking-wider text-[11px] border-b border-slate-800 font-mono">
                    <tr>
                      <th className="py-3.5 px-4">Asset Identifier &amp; Type</th>
                      <th className="py-3.5 px-3">Source Scanner</th>
                      <th className="py-3.5 px-3">Sensitivity / Criticality</th>
                      ...
                      <th className="py-3.5 px-4 text-right">Story &amp; Evidence</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/60 text-slate-200 font-sans">
                    {assets.map((asset) => (
                      <tr key={asset.asset_id} className="hover:bg-slate-800/40 transition-colors">
                        ...
                        {/* Source Scanner */}
                        <td className="py-3.5 px-3">{getSourceBadge(asset.source)}</td>

                        {/* Sensitivity / Criticality */}
                        <td className="py-3.5 px-3 capitalize">
                          <div className="text-slate-300 font-medium">{asset.data_sensitivity || 'internal'}</div>
                          <div className="text-[11px] text-slate-500">{asset.business_criticality || 'medium'}</div>
                        </td>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-900/90 text-slate-400 uppercase tracking-wider text-[11px] border-b border-slate-800 font-mono">
                  <tr>
                    ...
                    <th className="py-3.5 px-3">Location / Target</th>
                    <th className="py-3.5 px-3">Severity</th>
                    <th className="py-3.5 px-3">Quantum Relevance</th>
                    <th className="py-3.5 px-3">Mosca Status</th>
                    <th className="py-3.5 px-3">PQC Target</th>
                    <th className="py-3.5 px-4 text-right">Inspect Asset</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60 text-slate-200 font-sans">
                  {findings.map((f) => (
                    <tr key={f.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3.5 px-3 text-amber-300 font-medium">{f.quantum_relevance}</td>
                      ...
                      <td
                        className="py-3.5 px-3 text-cyan-300 font-mono text-[11px] max-w-[180px] truncate"
                        title={f.pqc_migration || f.recommendation_target}
                      >
                        {f.pqc_migration || f.recommendation_target || 'NIST ML-KEM / ML-DSA'}
                      </td>
