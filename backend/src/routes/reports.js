...
router.get("/executive", async (req, res, next) => {
  try {
    ...
    defaultAuditService.logEvent({
      ...
      actor: {
        id: req.user?.sub || req.user?.userId || req.auth?.user?.sub || req.auth?.userId || "user",
        username: req.user?.username || req.auth?.user?.username || req.auth?.username || "user",
        role: req.user?.role || req.auth?.role || req.tenantContext?.roles?.[0] || "user",
        ipAddress: req.ip,
      },
