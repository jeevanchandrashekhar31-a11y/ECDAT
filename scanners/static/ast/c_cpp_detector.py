    ...
    def _check_call(self, node: tree_sitter.Node):
        ...
        if func_text in ("EVP_md5", "EVP_md4", "EVP_md2"):
            self._add_finding(
                ...
                "critical",
                "OpenSSL",
                f"Weak hash {func_text}() used via OpenSSL EVP API",
            )
            return
        if func_text in ("EVP_sha1", "EVP_dss1"):
            self._add_finding(
                ...
                "high",
                "OpenSSL",
                f"Deprecated hash {func_text}() used via OpenSSL EVP API",
            )
            return
        if func_text in ("EVP_sha256", "EVP_sha384", "EVP_sha512", "EVP_sha3_256", "EVP_sha3_512"):
            ...
            self._add_finding(
                ...
                "low",
                "OpenSSL",
                f"Secure hash {func_text}() used via OpenSSL EVP API",
            )
            return
        ...
        if func_text in ("SHA1_Init", "SHA1"):
            self._add_finding(
                ...
                "high",
                "OpenSSL",
                f"Direct deprecated hash {func_text}() used",
            )
            return
        if func_text in ("SHA256_Init", "SHA256", "SHA384_Init", "SHA512_Init", "SHA512"):
            ...
            self._add_finding(
                ...
                "low",
                "OpenSSL",
                f"Direct secure hash {func_text}() used",
            )
            return
        ...
        if func_text.startswith("EVP_"):
            ...
            if any(w in func_text for w in ("des_ede3", "des_ede", "des_cbc", "des_cfb", "des_ecb", "desx")):
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    f"Insecure DES cipher {func_text}() used via OpenSSL",
                )
                return
            if "rc4" in func_text:
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    f"Insecure RC4 stream cipher {func_text}() used",
                )
                return
            if "bf_" in func_text or func_text.startswith("EVP_bf"):
                self._add_finding(
                    lineno,
                    "C_WEAK_CIPHER_BLOWFISH",
                    "Blowfish",
                    "weak_cipher",
                    "critical",
                    "OpenSSL",
                    f"Vulnerable Blowfish cipher {func_text}() used (Sweet32 vulnerability)",
                )
                return
            if "rc2_" in func_text:
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    f"Broken RC2 cipher {func_text}() used",
                )
                return
            ...
            if func_text.endswith("_ecb"):
                ...
                self._add_finding(
                    lineno,
                    "C_INSECURE_CIPHER_MODE_ECB",
                    f"{algo} [ECB]",
                    "insecure_cipher_mode",
                    "critical",
                    "OpenSSL",
                    f"Insecure ECB block cipher mode used in {func_text}()",
                )
                return
            ...
            if "aes_256_gcm" in func_text or "chacha20_poly1305" in func_text:
                ...
                self._add_finding(
                    ...
                    "low",
                    "OpenSSL",
                    f"Authenticated cipher {func_text}() used",
                )
                return
        ...
        if func_text in ("RSA_generate_key_ex", "RSA_generate_key"):
            ...
            if bits_val is not None:
                ...
                self._add_finding(
                    ...
                    "medium",
                    "OpenSSL",
                    f"RSA key generation detected via {func_text}",
                )
            return
        ...
        if func_text == "EC_KEY_new_by_curve_name":
            ...
            if any(w in curve_arg for w in ("secp224", "NID_secp224k1", "NID_secp224r1")):
                self._add_finding(
                    ...
                    "high",
                    "OpenSSL",
                    "Weak elliptic curve P-224 (insufficient security margin)",
                )
            else:
                ...
                self._add_finding(
                    ...
                    "medium",
                    "OpenSSL",
                    f"ECDSA with curve {curve_name} (Shor quantum vulnerable)",
                )
            return
        ...
        if func_text in ("EVP_PKEY_new_raw_private_key", "EVP_PKEY_new_raw_public_key") or "ED25519" in call_text:
            if "ED25519" in call_text:
                self._add_finding(
                    ...
                    "medium",
                    "OpenSSL",
                    "Ed25519 signature key detected",
                )
                return
        ...
        if func_text in ("SSL_CTX_set_verify", "SSL_set_verify"):
            ...
            if len(args) >= 2 and "SSL_VERIFY_NONE" in args[1]:
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    "TLS certificate verification disabled (SSL_VERIFY_NONE)",
                )
                return
        ...
        if func_text in ("SSL_CTX_set_min_proto_version", "SSL_set_min_proto_version"):
            ...
            if any(w in ver_arg for w in ("TLS1_VERSION", "TLS1_1_VERSION", "SSL3_VERSION", "DTLS1_VERSION")):
                ...
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    f"Insecure minimum TLS version configured ({ver_name})",
                )
                return
                ...
                self._add_finding(
                    lineno,
                    "C_SECURE_TLS_VERSION",
                    ver_name,
                    "tls_protocol",
                    "low",
                    "OpenSSL",
                    f"Secure minimum TLS version configured ({ver_name})",
                )
                return
        ...
        if func_text in ("SSL_CTX_set_cipher_list", "SSL_set_cipher_list"):
            ...
            if any(w in cipher_str.upper() for w in ("RC4", "3DES", "DES", "NULL", "MD5", "EXPORT")):
                self._add_finding(
                    lineno,
                    "C_INSECURE_CIPHER_SUITE",
                    "Insecure-CipherSuite",
                    "insecure_cipher_suite",
                    "high",
                    "OpenSSL",
                    f"Legacy or weak cipher suite configured: {cipher_str}",
                )
                return
        ...
        if func_text == "X509_sign":
            ...
            if "md5" in md_arg.lower():
                self._add_finding(
                    ...
                    "critical",
                    "OpenSSL",
                    "X509 certificate signed using broken MD5 digest",
                )
                return
                ...
                self._add_finding(
                    ...
                    "high",
                    "OpenSSL",
                    "X509 certificate signed using deprecated SHA-1 digest",
                )
                return
        ...
        if func_text.startswith("mbedtls_"):
            if "md5" in func_text:
                self._add_finding(
                    lineno,
                    "C_MBEDTLS_WEAK_HASH_MD5",
                    "MD5",
                    "weak_hash",
                    "critical",
                    "mbedTLS",
                    f"Weak mbedTLS MD5 hash used in {func_text}",
                )
                return
            if "sha1" in func_text:
                self._add_finding(
                    lineno,
                    "C_MBEDTLS_WEAK_HASH_SHA1",
                    "SHA-1",
                    "weak_hash",
                    "high",
                    "mbedTLS",
                    f"Deprecated mbedTLS SHA-1 hash used in {func_text}",
                )
                return
            if "des" in func_text:
                self._add_finding(
                    lineno,
                    "C_MBEDTLS_WEAK_CIPHER_DES",
                    "DES",
                    "weak_cipher",
                    "critical",
                    "mbedTLS",
                    f"Insecure mbedTLS DES cipher used in {func_text}",
                )
                return
            if "arc4" in func_text:
                self._add_finding(
                    lineno,
                    "C_MBEDTLS_WEAK_CIPHER_RC4",
                    "RC4",
                    "weak_cipher",
                    "critical",
                    "mbedTLS",
                    f"Insecure mbedTLS RC4 cipher used in {func_text}",
                )
                return
                ...
                if len(args) >= 4:
                    ...
                    if isinstance(raw, int) and raw < 2048:
                        self._add_finding(
                            ...
                            "critical" if raw <= 1024 else "high",
                            "mbedTLS",
                            f"Weak mbedTLS RSA key size ({raw} bits)",
                        )
                        return
            if func_text == "mbedtls_ssl_conf_authmode":
                if len(args) >= 2 and "MBEDTLS_SSL_VERIFY_NONE" in args[1]:
                    self._add_finding(
                        lineno,
                        "C_MBEDTLS_DISABLED_CERT_VALIDATION",
                        "TLS",
                        "disabled_certificate_validation",
                        "critical",
                        "mbedTLS",
                        "mbedTLS certificate verification disabled (MBEDTLS_SSL_VERIFY_NONE)",
                    )
                    return
            if func_text == "mbedtls_ssl_conf_min_version":
                if any(w in call_text for w in ("MBEDTLS_SSL_MINOR_VERSION_1", "MBEDTLS_SSL_MINOR_VERSION_0")):
                    self._add_finding(
                        lineno,
                        "C_MBEDTLS_INSECURE_TLS_VERSION",
                        "TLSv1.0",
                        "insecure_tls_protocol",
                        "critical",
                        "mbedTLS",
                        "mbedTLS configured with insecure minimum TLS version",
                    )
                    return
        ...
        if func_text.startswith("wc_") or func_text.startswith("wolfSSL_"):
            if any(w in func_text for w in ("Md5Hash", "InitMd5")):
                self._add_finding(
                    lineno,
                    "C_WOLFSSL_WEAK_HASH_MD5",
                    "MD5",
                    "weak_hash",
                    "critical",
                    "wolfSSL",
                    f"Weak wolfSSL MD5 hash used in {func_text}",
                )
                return
            if any(w in func_text for w in ("ShaHash", "InitSha")) and not any(
                w in func_text for w in ("256", "384", "512")
            ):
                self._add_finding(
                    lineno,
                    "C_WOLFSSL_WEAK_HASH_SHA1",
                    "SHA-1",
                    "weak_hash",
                    "high",
                    "wolfSSL",
                    f"Deprecated wolfSSL SHA-1 hash used in {func_text}",
                )
                return
            if "Des" in func_text:
                self._add_finding(
                    lineno,
                    "C_WOLFSSL_WEAK_CIPHER_DES",
                    "DES",
                    "weak_cipher",
                    "critical",
                    "wolfSSL",
                    f"Insecure wolfSSL DES cipher used in {func_text}",
                )
                return
                ...
                if len(args) >= 2:
                    ...
                    if isinstance(raw, int) and raw < 2048:
                        self._add_finding(
                            ...
                            "critical" if raw <= 1024 else "high",
                            "wolfSSL",
                            f"Weak wolfSSL RSA key size ({raw} bits)",
                        )
                        return
            if func_text == "wolfSSL_CTX_set_verify":
                if "WOLFSSL_VERIFY_NONE" in call_text or (len(args) >= 2 and "SSL_VERIFY_NONE" in args[1]):
                    self._add_finding(
                        lineno,
                        "C_WOLFSSL_DISABLED_CERT_VALIDATION",
                        "TLS",
                        "disabled_certificate_validation",
                        "critical",
                        "wolfSSL",
                        "wolfSSL certificate verification disabled (WOLFSSL_VERIFY_NONE)",
                    )
                    return
        ...
        if func_text.startswith("crypto_"):
            if func_text.startswith("crypto_secretbox"):
                self._add_finding(
                    lineno,
                    "C_LIBSODIUM_SECRETBOX",
                    "XSalsa20-Poly1305",
                    "symmetric_cipher",
                    "low",
                    "libsodium",
                    "libsodium secretbox (XSalsa20-Poly1305) authenticated encryption",
                )
                return
            if func_text.startswith("crypto_aead"):
                self._add_finding(
                    ...
                    "low",
                    "libsodium",
                    "libsodium AEAD (ChaCha20-Poly1305) authenticated encryption",
                )
                return
            if func_text.startswith("crypto_sign"):
                self._add_finding(
                    ...
                    "medium",
                    "libsodium",
                    "libsodium Ed25519 signature operation",
                )
                return
            if func_text.startswith("crypto_box") or func_text.startswith("crypto_scalarmult"):
                self._add_finding(
                    ...
                    "medium",
                    "libsodium",
                    "libsodium X25519 key exchange operation",
                )
                return
            ...
            if func_text.startswith("crypto_generichash"):
                self._add_finding(
                    ...
                    "low",
                    "libsodium",
                    "libsodium BLAKE2b generic hashing",
                )
                return
        ...
        if "Botan::" in func_text or "Botan::" in call_text:
            if "HashFunction::create" in func_text or "HashFunction::create" in call_text:
                ...
                if "MD5" in algo_arg or "MD4" in algo_arg:
                    self._add_finding(
                        lineno,
                        "CPP_BOTAN_WEAK_HASH_MD5",
                        "MD5",
                        "weak_hash",
                        "critical",
                        "Botan",
                        f"Weak Botan HashFunction '{algo_arg}' created",
                    )
                    return
                if "SHA-1" in algo_arg or "SHA1" in algo_arg:
                    self._add_finding(
                        lineno,
                        "CPP_BOTAN_WEAK_HASH_SHA1",
                        "SHA-1",
                        "weak_hash",
                        "high",
                        "Botan",
                        f"Deprecated Botan HashFunction '{algo_arg}' created",
                    )
                    return
                if "SHA-256" in algo_arg or "SHA-512" in algo_arg:
                    self._add_finding(
                        lineno,
                        "CPP_BOTAN_HASH_SECURE",
                        algo_arg,
                        "cryptographic_hash",
                        "low",
                        "Botan",
                        f"Secure Botan HashFunction '{algo_arg}' created",
                    )
                    return
            ...
            if "Cipher_Mode::create" in func_text or "Cipher_Mode::create" in call_text:
                ...
                if any(w in algo_arg for w in ("DES", "3DES", "RC4", "BLOWFISH")):
                    self._add_finding(
                        ...
                        "critical",
                        "Botan",
                        f"Weak Botan cipher mode '{algo_arg}' created",
                    )
                    return
                if "/ECB" in algo_arg or algo_arg.endswith("ECB"):
                    self._add_finding(
                        ...
                        "critical",
                        "Botan",
                        f"Insecure ECB cipher mode '{algo_arg}' in Botan",
                    )
                    return
                if "GCM" in algo_arg:
                    self._add_finding(
                        ...
                        "low",
                        "Botan",
                        f"Secure Botan AEAD cipher '{algo_arg}' created",
                    )
                    return
                ...
                if len(args) >= 2:
                    ...
                    if isinstance(raw, int) and raw < 2048:
                        self._add_finding(
                            ...
                            "critical" if raw <= 1024 else "high",
                            "Botan",
                            f"Weak Botan RSA private key ({raw} bits)",
                        )
                        return
        ...
        if func_text in ("rand", "srand"):
            self._add_finding(
                ...
                "low",
                "C-Stdlib",
                "Insecure pseudo-random number generator rand() used for potential crypto/security context",
            )
