...
class CSharpCryptoDetector:
    ...
    def _check_invocation(self, node: tree_sitter.Node):
        ...
        if (
            "MD5.Create(" in call_text
            or "new MD5CryptoServiceProvider(" in call_text
            or "MD5CryptoServiceProvider" in call_text
        ):
            self._add_finding(
                lineno,
                "CS_WEAK_HASH_MD5",
                ...
                "weak_hash",
                "critical",
                "Broken MD5 hash algorithm initialized via System.Security.Cryptography",
            )
            return
        if "SHA1.Create(" in call_text or "new SHA1CryptoServiceProvider(" in call_text or "SHA1Managed" in call_text:
            self._add_finding(
                ...
                "weak_hash",
                "high",
                "Deprecated SHA-1 hash algorithm initialized via System.Security.Cryptography",
            )
            return
        if any(w in call_text for w in ("SHA256.Create(", "new SHA256Managed(", "SHA256CryptoServiceProvider")):
            self._add_finding(
                ...
                "cryptographic_hash",
                "low",
                "Secure SHA-256 hash algorithm initialized",
            )
            return
        if any(w in call_text for w in ("SHA512.Create(", "new SHA512Managed(", "SHA384.Create(")):
            ...
            self._add_finding(
                lineno, "CS_HASH_SHA512", algo, "cryptographic_hash", "low", f"Secure {algo} hash algorithm initialized"
            )
            return

        # 2. Ciphers: TripleDES (checked before DES), DES, RC2, Rijndael, Aes
        if (
            "TripleDESCryptoServiceProvider" in call_text
            or "TripleDES.Create(" in call_text
            or "TripleDES" in call_text
        ):
            self._add_finding(
                lineno,
                "CS_WEAK_CIPHER_3DES",
                ...
                "weak_cipher",
                "critical",
                "Vulnerable TripleDES cipher initialized",
            )
            return
        if (
            "DESCryptoServiceProvider" in call_text
            or "DES.Create(" in call_text
            or re.search(r"\bDES\.Create\(", call_text)
        ):
            self._add_finding(
                lineno, "CS_WEAK_CIPHER_DES", "DES", "weak_cipher", "critical", "Broken DES cipher initialized"
            )
            return
        if "RC2CryptoServiceProvider" in call_text or "RC2.Create(" in call_text:
            self._add_finding(
                lineno, "CS_WEAK_CIPHER_RC2", "RC2", "weak_cipher", "critical", "Broken RC2 cipher initialized"
            )
            return
        if "Aes.Create(" in call_text or "AesManaged" in call_text:
            self._add_finding(
                lineno, "CS_CIPHER_AES", "AES", "symmetric_cipher", "low", "Standard AES cipher initialized"
            )
            return
        ...
        if "ECDsa.Create(" in call_text:
            ...
            if any(w in call_text for w in ("nistP224", "secp224")):
                self._add_finding(
                    ...
                    "weak_asymmetric_key",
                    "high",
                    "Weak elliptic curve P-224 configured",
                )
            else:
                curve = "P-256"
                ...
                self._add_finding(
                    ...
                    "asymmetric_key",
                    "medium",
                    f"ECDSA curve {curve} (Shor quantum vulnerable)",
                )
            return
        ...
        if "CertificateRequest" in call_text:
            if any(w in call_text for w in ("HashAlgorithmName.MD5", "MD5")):
                self._add_finding(
                    ...
                    "critical",
                    "X509 CertificateRequest configured with broken MD5 signature hash",
                )
                return
                ...
                self._add_finding(
                    ...
                    "weak_signature_algorithm",
                    "high",
                    "X509 CertificateRequest configured with deprecated SHA-1 signature hash",
                )
                return
    ...
    def _check_assignment(self, node: tree_sitter.Node):
        ...
        if "SecurityProtocol" in assign_text or "EnabledSslProtocols" in assign_text or "SslProtocols" in assign_text:
            if any(
                re.search(pat, assign_text)
                for pat in (r"\bSsl3\b", r"SecurityProtocolType\.Tls\b", r"SslProtocols\.Tls\b", r"\bTls11\b")
            ):
                proto = (
                    "TLSv1.0"
                    if ("Tls11" not in assign_text and "Ssl3" not in assign_text)
                    else ("SSLv3" if "Ssl3" in assign_text else "TLSv1.1")
                )
                self._add_finding(
                    lineno,
    ...
    def _add_finding(
        self, line_number: int, rule_id: str, algorithm: str, finding_type: str, severity: str, reason: str
    ):
        evidence_line = ""
