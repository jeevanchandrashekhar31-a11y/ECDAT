...
|---|---|---|---|---|
| **Python Core & Scanners** | [`requirements.txt`](requirements.txt) | [`requirements.lock`](requirements.lock) & [`requirements-lock.txt`](requirements-lock.txt) | 55 packages | Production Runtime |
| **Node.js Backend & Risk Engine** | [`backend/package.json`](backend/package.json) | [`backend/package-lock.json`](backend/package-lock.json) | 243 packages | Production Runtime + Dev |
| **React Frontend Dashboard** | [`frontend/package.json`](frontend/package.json) | [`frontend/package-lock.json`](frontend/package-lock.json) | 404 packages | Production Bundle + Dev |
| **Workspace Root Scripts** | [`package.json`](package.json) | [`package-lock.json`](package-lock.json) | 1 package | Workspace Orchestration |
| **Total Ecosystem Inventory** | — | — | **702 unique packages** | Complete Graph |

### 2.2 Generated SBOM Artifacts
- **Primary Standard**: CycloneDX 1.6 JSON ([`artifacts/sbom/ecdat_sbom_cyclonedx.json`](artifacts/sbom/ecdat_sbom_cyclonedx.json))
...
- **Secondary Interoperability Standard**: SPDX 2.3 JSON ([`artifacts/sbom/ecdat_sbom_spdx.json`](artifacts/sbom/ecdat_sbom_spdx.json))
...
ECDAT integrates [`scripts/scan_vulnerabilities.py`](scripts/scan_vulnerabilities.py):
...
- **Artifacts**: Emits [`artifacts/security/ecdat_vulnerability_report.json`](artifacts/security/ecdat_vulnerability_report.json) and [`artifacts/security/VULNERABILITY_REPORT.md`](artifacts/security/VULNERABILITY_REPORT.md).
- **Run Command**:
```bash
...
1. **Python Dependencies**: All direct and transitive Python packages are pinned to exact versions in [`requirements.lock`](requirements.lock).
2. **Node.js Dependencies**: Both [`backend/package-lock.json`](backend/package-lock.json) and [`frontend/package-lock.json`](frontend/package-lock.json) enforce immutable SHA-512 subresource integrity.
...
- **Engine**: [`scripts/sign_artifacts.py`](scripts/sign_artifacts.py)
- **Manifests**: Computes canonical `SHA256SUMS` and `SHA512SUMS` covering all SBOMs, security reports, and attestations.
- **Asymmetric Ed25519 Signing**: Digitally signs checksum manifests using Ed25519 asymmetric cryptography; signature stored at [`artifacts/SHA256SUMS.sig`](artifacts/SHA256SUMS.sig).
...
- **Engine**: [`scripts/generate_provenance.py`](scripts/generate_provenance.py)
- **Specification**: In-toto statement conforming to `https://slsa.dev/provenance/v1`.
- **Attestation File**: [`artifacts/provenance/ecdat_provenance.slsa.json`](artifacts/provenance/ecdat_provenance.slsa.json)
- **Attested Attributes**:
...
- **Least Privilege Workflow**: [`.github/workflows/ecdat-supply-chain.yml`](.github/workflows/ecdat-supply-chain.yml)
...
- **Automated Dependency Updates**: [`.github/dependabot.yml`](.github/dependabot.yml) monitors pip, npm (backend), npm (frontend), and GitHub Actions.
...
The Release Gate engine ([`scripts/release_gate.py`](scripts/release_gate.py)) evaluates 6 mandatory criteria before any build can be declared valid:

| Gate # | Condition | Enforcement Mechanism | Failure Trigger |
|---|---|---|---|
| **Gate 1** | **Critical Known Exploitable Dependencies** | Evaluates [`artifacts/security/ecdat_vulnerability_report.json`](artifacts/security/ecdat_vulnerability_report.json) against [`rules/security_exceptions.json`](rules/security_exceptions.json). | Any unexempted or expired critical vulnerability. |
...
| **Gate 6** | **Required SBOM Verification** | Inspects [`artifacts/sbom/ecdat_sbom_cyclonedx.json`](artifacts/sbom/ecdat_sbom_cyclonedx.json) and `spdx.json`. | Missing SBOM, empty file, or invalid CycloneDX 1.6 / SPDX 2.3 schema. |

### 4.2 Documented Security Exceptions Protocol
Exceptions must be recorded in [`rules/security_exceptions.json`](rules/security_exceptions.json) with:
